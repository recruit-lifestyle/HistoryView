...
public class FootprintsViewController: UIViewController {
    var rowNumber = 3
    ...
    public var titleLabel = UILabel(frame: CGRectMake(0.0, 20.0, UIScreen.mainScreen().bounds.width, 44.0))
    ...
    convenience init() {
        ...
        if let path = NSBundle(forClass: FootprintsViewController.self).pathForResource("HistoryView", ofType: "bundle") {
            let bundle = NSBundle(path: path)
            closeButton.image = UIImage(named: "close.png", inBundle: bundle, compatibleWithTraitCollection: nil)!.imageWithRenderingMode(UIImageRenderingMode.AlwaysTemplate)
